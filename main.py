...
def load():
    ...
    cur.execute("""SELECT s.student_id,s.first_name||' '||s.last_name,s.age,s.grade,s.registration_date,
                   g.guardian_name||'('||g.phone||')'
                   FROM students s LEFT JOIN guardians g ON s.guardian_id=g.guardian_id""")
    for r in cur.fetchall():
        sid=r[0]
        cur.execute("SELECT lesson_name FROM lessons l JOIN student_lessons sl ON l.lesson_id=sl.lesson_id WHERE sl.student_id=?",(sid,))
...
# إدارة الدروس
def manage_lessons():
    f=tk.Toplevel(root);f.geometry("300x300");f.title("Lessons")
    lb=tk.Listbox(f);lb.pack(fill="both",expand=True)
    e=tk.Entry(f);e.pack()

    def reload_lessons():
        lb.delete(0,"end")
        cur.execute("SELECT lesson_id,lesson_name FROM lessons")
        for l in cur.fetchall():lb.insert("end",f"{l[0]} - {l[1]}")

    def add_lesson():
        name=e.get().strip()
        if not name:messagebox.showerror("err","empty");return
        cur.execute("INSERT INTO lessons(lesson_name)VALUES(?)",(name,))
        con.commit();e.delete(0,"end");reload_lessons()
...
def form(edit=False,sid=None):
    f=tk.Toplevel(root);f.geometry("400x500")
    e1=tk.Entry(f);e2=tk.Entry(f);e3=tk.Entry(f)
    grades=["Grade 1","Grade 2","Grade 3","Grade 4","Grade 5","Grade 6"]
    grade=tk.StringVar();cb=ttk.Combobox(f,textvariable=grade,values=grades,state="readonly")
    e5=tk.Entry(f);e6=tk.Entry(f)
    tk.Label(f,text="First").pack();e1.pack()
    tk.Label(f,text="Last").pack();e2.pack()
    tk.Label(f,text="Age").pack();e3.pack()
    tk.Label(f,text="Grade").pack();cb.pack()
    tk.Label(f,text="Guardian").pack();e5.pack()
    tk.Label(f,text="Phone").pack();e6.pack()
    tk.Label(f,text="Lessons").pack()
    lb=tk.Listbox(f,selectmode="multiple");lb.pack(fill="both",expand=True)

    cur.execute("SELECT lesson_id,lesson_name FROM lessons");all_lessons=cur.fetchall()
    for l in all_lessons:lb.insert("end",f"{l[0]} - {l[1]}")

    if edit:
        cur.execute("""SELECT s.first_name,s.last_name,s.age,s.grade,g.guardian_name,g.phone
                       FROM students s LEFT JOIN guardians g ON s.guardian_id=g.guardian_id
                       WHERE s.student_id=?""",(sid,))
        d=cur.fetchone()
        if d:
            e1.insert(0,d[0]);e2.insert(0,d[1]);e3.insert(0,str(d[2]));grade.set(d[3]);e5.insert(0,d[4]);e6.insert(0,d[5])
        cur.execute("SELECT lesson_id FROM student_lessons WHERE student_id=?",(sid,))
        chosen=[x[0] for x in cur.fetchall()]
        for i,l in enumerate(all_lessons):
            if l[0] in chosen:lb.selection_set(i)

    def save():
        fn,ln,ag,gr,gn,gp=e1.get().strip(),e2.get().strip(),e3.get().strip(),grade.get(),e5.get().strip(),e6.get().strip()
        err=check(fn,ln,ag,gr,gn,gp)
        if err:messagebox.showerror("err",err);return
...
def edit_():
    s=tree.selection()
    if not s:messagebox.showwarning("warn","select");return
...
root=tk.Tk();root.geometry("1000x500");root.title("School System")
